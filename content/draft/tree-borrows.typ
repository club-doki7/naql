...
我们记录 `x` 与 `y` 的状态：它们皆由 `&mut *ptr` 创建，也就是说它们是从 `root` 间接派生的可变引用。引用 `y` 是 `x` 的兄弟，因此经由 `y` 的访问以 `x` 观之就是一次外部访问，反之亦然。执行 `*x = 13` 会引发一次对 `x` 的局部写入（使其变为 #p-unique）和一次 `y` 的外部写写入（使其变为 #p-disabled）。接着，`*y = 20` 是一次对 #p-disabled 节点的局部写入，而这是未定义行为，正如预期。
...
#bibliography("tree-borrows.bib", title: "参考文献")
