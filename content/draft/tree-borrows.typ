...
- 为支持两阶段借用，我们使用树代替栈来记录引用间的关系。我们知道，每个引用都有唯一的父级：创建它的引用 / 局部变量。但栈结构还强加了一个额外的约束：每个引用在同一时刻只能有一个有效子级。而这在处理两阶段借用时就捉襟见肘了：处于保留阶段的可变引用可与多个共享引用和平共处。这些引用必须被作为同一个父级引用的独立子级记录，而当可变引用被激活时，同级的引用就不能再使用了。这样一来，使用树形结构也就顺理成章。