...
Rust 程序设计语言因其基于所有权的类型系统而闻名，这一类型系统为内存安全和无畏并发提供了强有力的保证。然而，Rust 也提供了#tm_fst("非安全", "unsafe") 代码这一“逃生舱门”：在 `unsafe` 的领域中并不自动保障安全性，一切全凭程序设计者自觉。这便引出一种矛盾：一方面，编译器希望能尽可能利用类型系统的强力保证——特别是与指针别名相关的保证——以实现强大的过程内优化；另一方面，这些优化易因“行为不良”的非安全代码而失效。因此，要确保优化的正确性，就必须清晰地定义什么样的非安全代码算是“行为不良”。之前的工作——#tm[栈式借用模型]——为此定义了一系列规则。然而，栈式借用模型排除了几个在实际的 Rust 非安全代码中常见的模式，且并未考虑到 Rust 借用检查器近期引入的一些高级特性。

为解决这些问题，我们提出了#tm[树形借用模型]。顾名思义，树形借用模型将栈式借用模型的核心替换为树形结构。这就克服了先前所述的限制：我们评估了 30 000 个最常用的 Rust crate，树形借用模型拒绝的测试用例相比栈式借用模型减少了 54%。此外我们还（在 Rocq 中）证明了新模型在保留栈式借用模型的大部分优化的同时，还让一些重要的新优化成为了可能——特别是#tm_fst("读-取重排序", "read-read reordering")。
