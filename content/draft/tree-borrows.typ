...
我们不是第一批勇攀高峰的人：栈式借用模型 [12] 有着完全相同的目标。栈式借用模型注意到，在例 3 这样的场景中，当前有效的指针可被记录在一个栈中：`x` 在栈底，`ref` 在它上面，而 `ref2` 在栈顶。栈模型是对这一事实的建模：引用的所有使用必须是#tm_fst("良嵌套的", "well nested")。当 `ref1` 被使用时，`ref2` 就会失效。不幸的是，简单的栈模型有三大问题。

栈式借用模型的第一个问题在于它会阻止一些基本的优化，例如重排序相邻的读取操作。原因很简单：在例 3 中，若先读取 `ref2` 再读取 `ref1`，那么一切相安无事。`ref2` 是“嵌套于” `ref` 中的，所以所有访问都是允许的。然而，若重排序这两个操作，就成了先读取 `ref1` 再读取 `ref2`。这在栈式借用模型下是未定义行为，因为这违反了良嵌套规则。读取重排序是一项至关重要的优化，因此这是一个显著的弊端#footnote[除了整个换掉栈式借用模型之外，这一缺陷还可通过为优化所操作的中间表示形式选用不同的模型来克服。但这仍然需要开发一个新模型，而因为该模型是栈式借用模型的细化，其必然受到更多约束——即中间表示模型具有的未定义行为只能比表层模型更少。为此，我们决定完全体换掉栈式借用模型。]。

栈式借用模型的第二个问题在于它将引用限定在一个静态范围内：引用被创建之后，引用的类型决定了它所能访问的内存范围，无论是引用本身还是由它所派生出的指针，均不能访问这一范围外的内存。而在实践中，这往往并非程序设计者所期望的行为。例如，有 C 语言经验的程序设计者有时会创建指向数组首元素的引用，将其转成原始指针，然后用指针访问整个数组。栈式借用模型会拒绝这种操作，因为从指向数组首元素的引用派生出的原始指针只能访问这一个元素。程序设计者的心智和实际模型之间的这一差异导致现实代码中频繁出现未定义行为 [1, 3, 5, 6, 8]，因此一个更宽松的替代选项呼之欲出。

第三个问题则相当技术性，它涉及一个与栈式借用同期开发的 Rust 借用检查器特性：#tm_fst("二阶段借用", "two-phase borrows")。这一特性弱化了对某些可变引用的要求：它们不必在创建之后马上就是唯一的；相反，它们在被创建时处于一个#tm_fst("保留", "reserved") 阶段，这一阶段允许只读别名。只在可变引用第一次被写入前、被#tm_fst("激活", "activate / activation") 时，唯一性才被强制执行。

原本的栈式借用模型完全不支持两阶段借用。Rust 程序设计者用栈式借用模型来检查代码中的未定义行为，而栈式借用模型的实现只对二阶段借用提供了简陋的支持：在检查含有二阶段借用的代码时，实现基本上是把两阶段借用当作原始指针处理的。然而，这一简陋的支持会带来一些非常反直觉的后果，时常让 Rust 程序设计者感到困惑，如下例所示：
...
栈式借用模型接受以上程序。然而，若将 `write` 调用替换为其函数体，则栈式借用模型会拒绝该程序#footnote[这看似是栈式借用模型不允许内联，但实际的情况更加微妙。行为上的差异源于表层 Rust 被#tm_fst("繁饰", "elaborate") 为内部中间表示形式的方式不同；而中间表示形式级别的内联对于栈式借用模型而言是健全的。]。造成这一区别的原因在于 `write(x)` 隐式地展开成了 `write(&twophase x)`，这里 `&twophase` 是一个伪语法，表示受两阶段借用规则约束的隐式可变引用。而栈式借用模型会将两阶段即用当作原始指针处理。这一差异不但会让程序员感到困惑，将隐式创建的可变引用视作指针也会限制优化潜力。

== 我们的贡献：树形借用模型

在本文中，我们展示了一种能够克服以上局限的新别名模型，我们称之为*树形借用模型*：

- 为支持两阶段借用，我们使用树代替栈来记录引用间的关系。